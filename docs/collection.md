...
- 첫 운전 버튼에서 기록 선택 창을 먼저 연다. 새 방문자는 브라우저 보관·서버 전송 모두 OFF이며, 선택 확정 후 같은 첫 주행부터 적용한다. `기록 없이 시작`은 동일한 운전 기능을 제공한다. 안내 버전별 선택 완료를 기억하되 이는 전송 동의를 대신하지 않는다. 차고의 `기록 설정`에서 재선택·철회·삭제할 수 있다.
