...
- 학습: `parkside-open-v1`, `parkside-neighbors-v1`(train) + `parkside-pillar-v1`(validation) — 웹이 실제로 노출하는 세 템플릿([web/src/driving.ts](../web/src/driving.ts)) 그대로. #17 과적합 해결(시작 자세 흔들기+무작위 장애물 배치)을 거친 현재 학습 데이터는 train 103,035개, validation 50,913개 (observation, action) 쌍(`training/model/train-run.json`) — 최초 버전(seed 42, train 1092/validation 594)보다 훨씬 커졌다.
...
- 현재 artifact(seed 2)의 최종 train loss ≈ 0.000219(`training/model/train-run.json`). 체크포인트는 validation loss가 아니라 실제 폐루프 rollout 성공/충돌/오차로 고른다(같은 파일의 `checkpointSelection`) — 작은 지도학습 loss가 실제 주차 성공을 보장하지는 않는다는 원칙은 그대로다.
