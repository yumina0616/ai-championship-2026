...
> 현재 상태: 세 가지 프리셋 또는 직접 편집한 맵에서 P/R/D·드래그 핸들·페달로 직접 운전하거나 실제 학습 모델의 운전을 볼 수 있습니다. 차량·충돌·거리 센서·평가 엔진과 센서 HUD·경고음·후진 조향 연장선을 연결했습니다. 로컬 최근 5회 기록·상태 재생·동일 환경 비교·맵 포함 링크/JSON 공유를 제공합니다. 마스코트는 실험용 초기 모델이며 소규모 heldout 성공 0/1로, 주차 성능을 보장하지 않습니다. Cloudflare 공개 테스트 웹과 비공개 R2 연결을 검증했지만 실제 사용자 서버 수집은 꺼져 있습니다. 개발용 합성 기여 API는 공개하지 않습니다.
...
운전 중에는 차체·위치·각도·정지 조건과 부족한 이유가 표시됩니다. 차체 전체를 목표칸 안에 놓고 1초 정지하면 P 마무리 안내가 나타납니다(이미 P라면 성공 화면). 시간 초과에는 마지막 미충족 이유를 보여줍니다. 웹 연습 프리셋 v2의 치수와 판정 기준은 [웹 개발 안내](web/README.md#주차-판정과-웹-연습-프리셋-v2)를 참고하세요.
...
공개 테스트: [PARKSIDE](https://parkside-parking.jinhyeong9568-663.workers.dev). 실제 사용자 서버 수집은 꺼져 있습니다. 배포 설정·검증·남은 점은 [Cloudflare 배포 안내](docs/cloudflare-deployment.md)를 따릅니다. CI artifact 제공과 공개 배포는 별개입니다.

| 담당 | 지금 할 일 | 다음 연결 |
| --- | --- | --- |
| JH-9568 | #12/#14 main 통합, Cloudflare 공개 테스트 배포 | #13 실제 수집 경계와 #15 실기기·운영 검증 |
